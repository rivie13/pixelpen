...
func initialized(p_size : Vector2i, p_name : String = "Untitled", p_file_path : String = "", one_layer : bool = true):
	...
	project_name = p_name
	canvas_size = p_size
	animation_fps = _pixelpen().state.userconfig.default_animation_fps
...
func set_mode(mode : ProjectMode, mask : Image = null):
	...
	_pixelpen().state.edit_mode_changed.emit(mode)
...
func export_animation_gif(path : String):
	...
	var exporter = _pixelpen().state.GIFExporter.new(canvas_size.x, canvas_size.y)
	...
	for cell in animation_timeline:
		...
		exporter.add_frame(img, 1.0 / animation_fps, _pixelpen().state.MedianCutQuantization)
...
func update_onion_skin_images():
	...
	for i in _pixelpen().state.userconfig.onion_skin_total:
		prev_idx -= 1
...
func reset_brush_to_default():
	...
	_pixelpen().state.userconfig.brush.clear()
	for i in range(0, 16):
		...
		_pixelpen().state.userconfig.brush.push_back(image)
		_pixelpen().state.userconfig.save()


func reset_stamp_to_default():
	_pixelpen().state.userconfig.stamp.clear()
	_pixelpen().state.userconfig.save()
